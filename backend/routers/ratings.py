...
from backend.routers.memos import get_current_iso_time, get_memo_with_permission
...
def list_axes(memo_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """指定メモの評価軸一覧を取得する。"""
    # 閲覧権限の確認
    get_memo_with_permission(db, memo_id, current_user.id, 'read')
...
def create_axis(memo_id: int, data: RatingAxisCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """指定メモに新しい評価軸を追加する。"""
    # 編集権限の確認
    get_memo_with_permission(db, memo_id, current_user.id, 'write')
...
def update_axis(axis_id: int, data: RatingAxisUpdate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """評価軸の名前やメソッドを更新する。"""
    ...
    # 編集権限の確認
    get_memo_with_permission(db, axis.memo_id, current_user.id, 'write')
...
def delete_axis(axis_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
...
def upsert_rating(axis_id: int, data: RateRequest, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """評価を登録または更新する（Upsert）。"""
    axis = db.query(RatingAxisModel).filter(RatingAxisModel.id == axis_id).first()
    if not axis:
        raise HTTPException(status_code=404, detail='Axis not found')
    # 閲覧権限の確認
    get_memo_with_permission(db, axis.memo_id, current_user.id, 'read')
    score = normalize_score(data.raw_value, axis.method)
    existing = db.query(MemoRatingModel).filter(
        MemoRatingModel.axis_id == axis_id, MemoRatingModel.user_id == current_user.id
    ).first()
    if existing:
        ...
        rating = MemoRatingModel(
            axis_id=axis_id, user_id=current_user.id,
...
def delete_rating(axis_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """現在のユーザーの評価を削除する。"""
    axis = db.query(RatingAxisModel).filter(RatingAxisModel.id == axis_id).first()
    if not axis:
        raise HTTPException(status_code=404, detail='Axis not found')
...
def get_memo_ratings(memo_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
...
def get_visibility(memo_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """指定メモの可視性グリッドを取得する。"""
    # 閲覧権限の確認
    get_memo_with_permission(db, memo_id, current_user.id, 'read')
    ...
    for u in users:
        ...
        for ax in axes:
            vis = db.query(RatingVisibilityModel).filter(
                RatingVisibilityModel.viewer_user_id == current_user.id,
        ...
        grid.append(row)
    return {'viewer_user_id': current_user.id, 'grid': grid}

@router.put('/visibility/toggle')
def toggle_visibility(data: VisibilityToggle, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """個別の可視性を切り替える。"""
    axis = db.query(RatingAxisModel).filter(RatingAxisModel.id == data.axis_id).first()
    if not axis:
        raise HTTPException(status_code=404, detail='Axis not found')
    # 閲覧権限の確認
    get_memo_with_permission(db, axis.memo_id, current_user.id, 'read')
    ...
    if existing:
        ...
        vis = RatingVisibilityModel(
            viewer_user_id=current_user.id, target_user_id=data.target_user_id,
...
def bulk_visibility(data: VisibilityBulk, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    """可視性を一括で切り替える。"""
    if data.axis_id:
        axis = db.query(RatingAxisModel).filter(RatingAxisModel.id == data.axis_id).first()
        if not axis:
    ...
    if data.mode == 'all_on':
        db.query(RatingVisibilityModel).filter(RatingVisibilityModel.viewer_user_id == current_user.id).update({'visible': 1}, synchronize_session=False)
    elif data.mode == 'all_off':
        db.query(RatingVisibilityModel).filter(RatingVisibilityModel.viewer_user_id == current_user.id).update({'visible': 0}, synchronize_session=False)
    elif data.mode == 'user_on' and data.target_user_id:
        db.query(RatingVisibilityModel).filter(RatingVisibilityModel.viewer_user_id == current_user.id, RatingVisibilityModel.target_user_id == data.target_user_id).update({'visible': 1}, synchronize_session=False)
    elif data.mode == 'user_off' and data.target_user_id:
        db.query(RatingVisibilityModel).filter(RatingVisibilityModel.viewer_user_id == current_user.id, RatingVisibilityModel.target_user_id == data.target_user_id).update({'visible': 0}, synchronize_session=False)
    elif data.mode == 'axis_on' and data.axis_id:
        db.query(RatingVisibilityModel).filter(RatingVisibilityModel.viewer_user_id == current_user.id, RatingVisibilityModel.axis_id == data.axis_id).update({'visible': 1}, synchronize_session=False)
    elif data.mode == 'axis_off' and data.axis_id:
        db.query(RatingVisibilityModel).filter(RatingVisibilityModel.viewer_user_id == current_user.id, RatingVisibilityModel.axis_id == data.axis_id).update({'visible': 0}, synchronize_session=False)
    db.commit()
...
def ratings_summary(memo_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
